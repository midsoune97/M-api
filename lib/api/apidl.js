...
function watuksolatmy(q) {
    return new Promise((resolve, reject) => {
		axios.get('https://fm6oa.org/prieres/code/index.php?ville=' + q)
		.then(({ data }) => {
		  const $ = cheerio.load(data)
		  let hasil = []
		  $('.prayerAdan > tbody > tr').each(function (a, b) {
			hasil.push({
			   sobh: $(b).find('td:nth-child(1)').text(),
			   dohr: $(b).find('td:nth-child(2)').text(),
			   asr: $(b).find('td:nth-child(3)').text(),
			   maghreb: $(b).find('td:nth-child(4)').text(),
			   icha: $(b).find('td:nth-child(5)').text()
	  
		  })  
		  })
 let mdina = $('body > container > prayer > prayer-header.col-lg-12 > prayer-titre.col-lg-9.col-xs-9.col-sm-9 > h2').text()
